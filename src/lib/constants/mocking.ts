export const MAIN_MOCKING = {
  id: 0,
  username: "아이어를위하여",
  img: "/iu_demo.jpeg",
  role: "general",
  createdAt: "2024-06-12T05:34:39.285Z",
  fandom: "유애나",
  news: [
    {
      1: "아이유 vs 젤나가 누가 제일 잘나가나?",
      2: "아이유, '제라툴 맞짱뜨자' 발언!",
      3: "속보! 아이유, 제라툴이랑 맞짱!",
    },
  ],
  recently: [
    { latter: "/vercel.svg", title: "칼날여왕 ㅗdfㅗ" },
    { latter: "/vercel.svg", title: "칼날여왕 ㅗㅗ" },
    { latter: "/vercel.svg", title: "칼날여왕 ㅗdfdsffㅗ" },
    { latter: "/vercel.svg", title: "칼날여왕 " },
  ],
};

export const MAIL_MOCKING = {
  title: "아이유로부터 답장",
  letterImageUrl: "url",
  originalText: [
    "*(안녕)*하세요",
    "아이유 입니다",
    "*(감사)*합니다",
    "*(사과)*에 *(안녕)*과 *(천재)*와 *(연예인)*이 있습니다.",
    "*(바보)*에 *(유쾌함)*과 *(소고기)*와 *(컵)*이 있습니다.",
    "*(영어)*에 *(달콤함)*과 *(돼지고기)*와 *(리모컨)*이 있습니다.",
    "*(친구)*에 *(지갑)*과 *(키워드2)*와 *(비타민)*이 있습니다.",
    "*(컴퓨터)*에 *(게임)*과 *(스피커)*와 *(휴대폰)*이 있습니다.",
    "*(인터넷)*에 *(놀이)*과 *(우주)*와 *(사람)*이 있습니다.",
  ],
  translatedText: [
    "Hello",
    "I am IU.",
    "Thank you",
    "english english english sentences",
    "english english english sentences",
    "english english english sentences",
    "english english english sentences",
    "english english english sentences",
    "english english english sentences",
  ],
  createdAt: "2024-06-14T06:17:45.880Z",
  sender: "아이유",
};

export const ARTIST = ["아이유", "방탄소년단"];
